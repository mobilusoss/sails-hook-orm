...
module.exports = function initialize(hook, sails, done){
  ...
  async.auto({
    ...
    _mergeInProgrammaticModuleDefs: ['_loadModelsAndCustomAdapters',function (async_data, next) {
      _.assignIn(hook.models, sails.config.orm.moduleDefinitions.models);
      _.assignIn(hook.adapters, sails.config.orm.moduleDefinitions.adapters);
      return next();
    }],
    ...
    _determineRelevantDatastoreIdentities: ['_mergeInProgrammaticModuleDefs', function (async_data, next){
      ...
      _.each(sails.config.connections, function _eachDatastoreConfig(datastoreConfig, datastoreIdentity) {
        ...
        var isDatastoreInUse = _.some(hook.models, function eachModel(modelDef) {
    ...
    _normalizeModelDefs: ['_validateDatastoreConfigsAndBuildAccessors', function (async_data, next) {
      try {
    ...
    _productionCheck: ['_normalizeModelDefs', function (async_data, next) {
      try {
    ...
    _doubleCheckMigration: ['_productionCheck', function (async_data, next) {
